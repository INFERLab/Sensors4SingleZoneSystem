...
for house_id, single_house_data in five_houses.items():
    ...
    # Create 'Night' column
    single_house_data.loc[:, 'Night'] = single_house_data['time'].apply(lambda x: x.date() if x.hour >= 7 else (x - pd.Timedelta(days=1)).date())

    # Initialize dictionary for the house
    valid_periods_dict[house_id] = {}

    for sensor in sensor_columns:
        # Identify when HVAC is off and when CoolingEquipmentStage1_RunTime is NaN
        single_house_data.loc[:, 'HVAC_Off'] = single_house_data['HeatingEquipmentStage1_RunTime'].isna() & single_house_data['CoolingEquipmentStage1_RunTime'].isna()

        # Identify groups of rows where HVAC is off. 
        single_house_data.loc[:, 'Off_Period'] = (single_house_data['HVAC_Off'] & (~single_house_data['HVAC_Off'].shift(1).fillna(False))).cumsum()

        for (night, period), period_data in single_house_data.groupby(['Night', 'Off_Period']):
            # Check if the period is during the night, has correct length, sufficient temperature change and no NaN values
            ...
            if is_night_time.all() and is_correct_length and is_temp_change_sufficient and no_nan_values:
